...
def test_sparse_position_permutation_does_not_renumber_rope(fusion):
    ...
    reordered = model(
        seed, embedding, head, k[..., perm, :], v[..., perm, :], pos[perm], 12
    )
...
def test_only_arrived_kv_receives_gradient_and_no_future_labels_are_inputs():
    ...
    pos = torch.tensor([0, 3, 8, 11])
    logits = model(
        torch.tensor([[7]]), embedding, head, k[..., pos, :], v[..., pos, :], pos, 12
    )
...
def test_progressive_reranker_stays_inside_base_topk():
    ...
    proposal = model.propose(seed, embedding, head, key, value, torch.arange(9), 9)
    inherited = head(
        model.sparse_hidden(seed, embedding, key, value, torch.arange(9), 9)
    ).argmax(-1)
    ...
    torch.testing.assert_close(
        model.propose_prepared_rerank(prepared, embedding, head), proposal
    )
    repaired = model.propose_prepared_rerank(
        prepared, embedding, head, first_target_token=proposal[:, :1]
    )
    torch.testing.assert_close(repaired, proposal[:, 1:])


def test_views_are_nested_and_account_for_protected_pages():
    order = page_order(19, 4, "random", 22)
    previous = set()
    for fraction in [0.05, 0.1, 0.2, 0.5, 1]:
...
def test_exact_streaming_attention_with_gqa_and_out_of_order_pages():
    ...
    for start, end in [(7, 13), (0, 7), (13, 19)]:
        state = merge_attention(
            state, attention_state(q, k[..., start:end, :], v[..., start:end, :])
        )
...
def test_anchor_schedule_does_not_duplicate_payload_bytes():
    ...
    anchored, anchor_ready = arrivals(payload, 0.1, bandwidth, True)
    ...
    assert anchor_ready == pytest.approx(
        5 * 0.1 * (payload / 36) * 8 / (bandwidth * 1e6)
    )
    for prefix_layers in range(37):
        switched, _ = switch_arrivals(payload, 0.1, bandwidth, prefix_layers)
...
def test_shape_invariant_attention_matches_explicit_causal_rows():
    ...
    for row in range(3):
        ...
        probability = torch.softmax(
            query[:, :, row : row + 1]
            @ expanded_key[:, :, :visible].transpose(-1, -2)
            * (8**-0.5),
            dim=-1,
            dtype=torch.float32,
        ).to(query.dtype)
