...
Status: preregistered working ledger, 2026-08-27. A row marked `missing` or
...
|---|---|---|---|---|
| C0 | the shared P/D state machine is exact by construction | hidden producer seed; protected prompt boundary arrives in atomic S1; no draft output; full arrival before exactly one verifier; target cache after verify; worker input chain matches scheduler state | scheduler, protected-anchor transfer/attention, and link-trace unit tests; a decoder-only 64K `n=2` core diagnostic passed 4/4 method requests; a second five-schedule-by-two-arm diagnostic passed 20/20 requests, 160/160 sparse forwards, and 20/20 verifier batches with exact token IDs, contiguous positions, and seed-position rewind | live mechanism passed; scale pending |
...
| C2 | overlap improves end-to-end latency in a realistic transfer-bound regime | n=100 paired baseline/fixed/continuous cells at 25/50 Gbps; P50/P95/P99; paired 95% CI; >=1.10x primary speedup | one valid 64K/25-Gbps `n=4` cell is negative: fixed and continuous are 76.7 and 37.1 ms slower than baseline; the first `n=100` run was invalidated by an L1 handoff eviction and the 128-GiB retry was deliberately stopped for input audit after 39 result rows, so neither is paper evidence | missing |
...
| C5 | the exact endpoint preserves task quality | 100% greedy token equality primary gate; official LongBench/LongBench-v2 metrics; per-dataset paired deltas | 3,653 audited natural-quality inputs per model for Llama and Qwen; no generation result | missing |
...
| C7 | gains are not an artifact of an unfair link or P-side cache hit | P=`kv_producer`, D=`kv_consumer`; identical total logical bytes/rate/wire time for every arm; protected S1 bytes charged; exclusive GPUs | roles fixed; monolithic/progressive fair-link pacing, atomic protected-anchor, and trace gates tested; the valid 64K `n=4` cell passes these live gates, while larger confirmation remains pending | preliminary live proof |
